...
void PropTarget(Node*& headPtr, int target) {
   ...
   // If target DNE, append to the end of list
   if (!targetFound) {
   ...
   // Exist? Rearrange nodes so that all nodes with target value are @ front
   Node* targetHead = nullptr;   // head, containing target
   Node* targetTail = nullptr;   // tail, containing target
   Node* nonTargetHead = nullptr; // head, not containing target
   Node* nonTargetTail = nullptr; // tail, not containing target

   cursor = headPtr;
   while (cursor) {
      Node* nextNode = cursor->link;
      cursor->link = nullptr;
      if (cursor->data == target) {
            if (!targetHead) {
               targetHead = cursor;
               ...
               targetTail->link = cursor;
               targetTail = cursor;
            }
      } else {
            if (!nonTargetHead) {
               nonTargetHead = cursor;
               nonTargetTail = cursor;
            } else {
               nonTargetTail->link = cursor;
               nonTargetTail = cursor;
            }
      }
      cursor = nextNode;
    ...
    // Attach target sublist to the front of the non-target sublist
   targetTail->link = nonTargetHead;
   headPtr = targetHead; // Update  headPtr to point to new head of list
}
